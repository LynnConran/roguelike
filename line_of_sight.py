...
def __compute(octant, origin, range_limit, x, top, bottom, floor_plan, hidden_map, is_player):
    while x <= range_limit:
        ...
        y = top_y
        while y >= bottom_y:
            if range_limit < 0 or math.dist([x], [y]) <= range_limit:
...
def blocks_light(x, y, octant, origin, floor_plan):
    ...
    if octant == 0:
        ...
        nx += x
        ny += y
    return not display.check_walls_and_doors(nx, ny, floor_plan)
